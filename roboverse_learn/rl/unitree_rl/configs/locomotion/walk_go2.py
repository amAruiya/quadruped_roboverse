...
@configclass
class WalkGo2EnvCfg(BaseEnvCfg):
    ...
    class RewardsScales:
        ...
        # Go2 的站立高度明显低于人形机器人 G1，通常在 0.28-0.35m 之间
        base_height = (-10.0, {"target_height": 0.32})
        joint_acc = -2.5e-7
        joint_vel = -0.001
        action_rate = -0.01
        ...
        # 针对 Go2 的足部打滑惩罚，Go2 的足部 link 名称通常包含 "foot"
        feet_slide = (-0.2, {"body_names": (".*foot")})
    ...
    callbacks_setup = {
        "material_randomizer": MaterialRandomizer(
            obj_name="go2",  # 对应 Go2Cfg 中的 name
        ...
        "mass_randomizer": MassRandomizer(
            obj_name="go2",
            body_names="base_link", # 四足机器人通常对 base 进行质量随机化
...
@configclass
class WalkGo2RslRlTrainCfg(RslRlOnPolicyRunnerCfg):
